...
D <- 15
N_obs <- 58
p1 <- simsP(Type = "GD", gd_alpha = seq(3,3.3, by = 0.01), gd_beta =  rep(30,30), D = D, days = 58)
...
stan_data <- list(N_obs = N_obs, D = D + 1, Y = round(data$case_reported_acc))
...
N_t_posterior_map <- function(lambda_t_samples, b_samples, D) {
  ...
  for (t in 1:time_length) {
    ...
    N_t_estimates[,t] <- rpois(sampling_size,lambda_t_samples[,t] * q_d_map)
...
round(apply(N_t_map_estimates, 2, mean) - apply(data$case_true, 1, mean),2) 




################# try p3
data3 <- simsDataGen(p = p3, days = N_obs)
stan_data_3 <- list(N_obs = N_obs, D = D + 1, Y = round(data3$case_reported_acc))
...
round(apply(N_t_map_estimates_3, 2, mean) - apply(data3$case_true, 1, mean),2) 
