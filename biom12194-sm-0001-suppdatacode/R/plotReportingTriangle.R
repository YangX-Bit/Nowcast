...
plotReportingTriangle <- function(nc) {
    ...
    plot(NA,xlim=c(0,D+2),ylim=range(1:length(range.dates)),axes=FALSE,ylab="",xlab="Delay (days)")
    axis(1,at=0:D)
    axis(2,at=1:length(range.dates),label=names.arg,las=1,cex.axis=theCex)
    axis(1,at=D+2, lab=expression(N(t,T)),cex.axis=theCex)
